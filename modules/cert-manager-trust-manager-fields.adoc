...
The following table lists the parameters for configuring how trust-manager handles logging, expired certificates, and trusted certificate authority (CA) bundles.
...
|`object`
|Optional. Configures the behavior of Operator for creating trust manager resources.
...
[NOTE]
====
To ensure the `useDefaultCAs: true` setting in trust bundle CR includes a package of publicly trusted CA certificates in your `Bundle` CR, you must set this value to `Enabled`.
====
...
|`array`
|A list of specific secret names that trust manager is authorized to create and update.
+
[NOTE]
====
