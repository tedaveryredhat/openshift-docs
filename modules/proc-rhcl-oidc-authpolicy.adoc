...
kind: AuthPolicy
metadata:
  name: _<talker_api_oidc_auth>_
  namespace: _<oidc_apps>_
spec:
  targetRef:
    group: gateway.networking.k8s.io
    kind: HTTPRoute
    name: talker-api-route
  rules:
    authentication:
      "oidc-idp-auth":
        jwt:
...
* Replace the `metadata.name` parameter value with the name you want to use for your `AuthPolicy` CR.
...
----
$ oc apply -f _<talker_api_oidc_auth.yaml>_
----
+
* Replace `_<talker_api_oidc_auth.yaml>_` with your `AuthPolicy` YAML filename.
