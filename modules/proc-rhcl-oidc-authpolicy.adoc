...
[role="_abstract"]
You can configure a {prodname} `AuthPolicy` custom resource (CR) to use an OIDC provider. You can ensure that different clusters exposing the same API can authenticate with the same permissions.
