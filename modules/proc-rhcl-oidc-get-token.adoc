...
[role="_abstract"]
After you configured a {prodname} `AuthPolicy` custom resource (CR) to use an OIDC provider, you must get a token. The following example uses {keycloak}.
...
* You have a demo OAuth2 client with a Direct Access grant enabled.
