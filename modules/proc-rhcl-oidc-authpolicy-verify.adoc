...
[role="_abstract"]
You can verify a {prodname} `AuthPolicy` custom resource (CR) after you applied to ensure that it is working as intended.
...
----
$ oc describe authpolicy _<talker_api_oidc_auth>_ -n _<oidc_apps>_
----
* Replace `_<talker_api_oidc_auth.yaml>_` with the name of your `AuthPolicy` CR.
* Replace `_<oidc_apps>_` with the namespace where your `AuthPolicy` CR is applied.
+
.Example output
[source,terminal]
----
